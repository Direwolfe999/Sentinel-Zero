...
import { Hero, ModeSelector, InputArea, ResultCard, useToast } from '@/components';
import { DemoScenarios } from '@/components';
import { ShimmerLoader } from '@/components';
...
export default function Page() {
  ...
  const handleAnalyze = async (textToAnalyze?: string, modeOverride?: AnalysisMode) => {
    const finalText = textToAnalyze || text;
    const finalMode = modeOverride || mode;

    if (!finalText.trim()) { 
      addToast('Please enter text', 'error'); 
      return; 
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        // Save to history
        saveToHistory(finalText, finalMode, data.data);
      } else {
  ...
  const handleDemoClick = (demoText: string) => {
    const detectedMode = detectMode(demoText);
    setText(demoText);
    setMode(detectedMode);
    
    // Auto-trigger analysis after a brief delay
    setTimeout(() => {
      handleAnalyze(demoText, detectedMode);
    }, 100);
  };

  const saveToHistory = (inputText: string, analysisMode: AnalysisMode, analysisResult: AnalysisResult) => {
    try {
        ...
        <motion.div exit={{opacity:0,y:-100}} transition={{duration:0.6}}>
          ...
          <div className="flex justify-center pb-20">
            <motion.button 
              ...
              whileTap={{scale:0.95}} 
              className="px-8 py-4 rounded-lg bg-gradient-to-r from-blue-500 to-cyan-400 text-white font-bold shadow-lg hover:shadow-xl transition-shadow text-lg"
            >
              Start Analyzing
            </motion.button>
          </div>
        ...
        >
          <div className="w-full max-w-3xl mb-12">
            <motion.button 
              ...
              whileHover={{scale:1.05}} 
              whileTap={{scale:0.95}} 
              className="mb-8 px-4 py-2 rounded-lg bg-slate-800 text-slate-300 hover:text-white transition-colors flex items-center gap-2"
            >
              ← Back to Home
            </motion.button>
            <h2 className="text-4xl font-black bg-gradient-to-r from-blue-400 to-cyan-300 bg-clip-text text-transparent">
              Sentinel Zero Analyzer
            </h2>
            <p className="text-slate-400 mt-2">
              Select a mode, enter input, and get AI-powered analysis
            </p>
          </div>
          ...
          <div className="w-full max-w-3xl space-y-8">
            ...
            {!result ? (
              <>
                {/* Demo Scenarios - Only show when no analysis yet */}
                <DemoScenarios onSelect={handleDemoClick} />
                ...
                {loading ? (
                  ...
                  <motion.button 
                    ...
                    whileTap={{scale:0.95}} 
                    disabled={!text.trim()} 
                    className="w-full px-8 py-4 rounded-lg bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white font-bold shadow-lg hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed text-lg"
                  >
                    🔍 Analyze Now
                  </motion.button>
                )}
